...
var __repositoryExists = function (opts /*, cb */) {
    ...
    var repo   = path.join(opts.path, '.git'),
        exists = true;

    if (!fs.existsSync(opts.path) || !fs.existsSync(repo)) {
        exists = false;
    }
    return exists;
};
...
GitContributors.prototype.list = function (git_project, cb) {
    ...
    var opts = { timeout: 5000, cwd: '.', maxBuffer: 5000 * 1024 };

    opts = _.merge(opts, __parseOpts(git_project));

    if (!__repositoryExists(opts /*, cb*/)) {
      return cb(
        new Error('Could not find .git repository at "'+ opts.path +'"'), null
      );
    }
