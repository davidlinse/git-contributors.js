...
var __parseOpts = function() {
  'use strict';

  var deferred = Q.defer();

  if (_.isObject(program)) {
    program = _.merge(opts, program);
  }
  else if (_.isString(program)) {
    opts.cwd = program;
  }
  else {
    deferred
      .reject(new Error('Something went wrong while building target path.'));
  }
